...
ws = sheet('01_요약', '올리브영 PB 시딩 성과 요약',
           '집계 26.09.07 · 시딩 2,139건 · 청구 840,514,264원(확정) · 조회수 56,612,765 · 매출 1,420,018,525원', DEEP)
...
KPI = [
    ...
    ('■ 비용', [('세금계산서 총액', 840514264, MON, '원'), ('크리에이터 원고료', 492408205, MON, '원'),
                ('건당 청구단가', 307481, MON, '원'), ('건당 원고료', 240199, MON, '원'),
                ('마크업', '25% (일부 15%)', None, '')]),
...
for nm, q, a, vw, mth, memo in [
    ('국내 (KR)', 1972, 778718264, 51550844, '25.04 ~ 26.08', '올영PB 11개 브랜드 · 2025 계약 272건 포함'),
...
line(ws, r, ['합계', f'=SUM(B{s0}:B{r-1})', f'=B{r}/$B${r}', f'=SUM(D{s0}:D{r-1})',
             ...
             '진행 기준 총계 · 세금계산서 발행 확정 840,514,264 + JP 미발행 8,904,000'],
...
note(ws, r, [
    ...
    '[금액] 세금계산서 발행 총액 840,514,264원(26.09.07 확정 · 26.02~08 발행분이 전부이며 2025 진행분 정산·바이오힐보 US 포함) 기준',
...
BD = [
    ...
    ('바이오힐보 (해외·US)', 0, 68, 30000000, 4739117, .005093, 0, 0, '25.08 US 68건 중 36건 성과 확보'),
...
sub_r = r; r += 1
line(ws, r, ['확정 조정', None, None, None, 40431189, None, None, None, None, None, None,
             '발행 확정 총액 − 프로젝트별 내역 차이 · 브랜드 귀속 확인필요 (A1·A4)'], fm, wrap=12)
adj_r = r; r += 1
line(ws, r, ['2025 시딩 계약', None, None, None, None, None, None, None, None, None, None,
             '25.04~26.01 진행 272건 · 정산액은 위 확정 총액에 포함 (A3 참조)'], fm, wrap=12); r += 1
line(ws, r, ['합계', f'=B{sub_r}', f'=C{sub_r}', f'=D{sub_r}', f'=E{sub_r}+E{adj_r}',
             f'=IFERROR(E{r}/D{r},"")', f'=G{sub_r}', f'=H{sub_r}', f'=I{sub_r}', f'=J{sub_r}',
             f'=IFERROR(J{r}/I{r},"")', '세금계산서 발행 확정 840,514,264원과 일치'],
...
note(ws, r, [
    ...
    '· 바이오힐보 US 68건은 청구 30,000,000원 전액을 계상하고 조회수는 성과 확보 36건분(4,739,117뷰)만 넣었다. 건당 조회수는 국내의 4.7배.',
    '· 2025 시딩 계약(272건)의 정산액 59,830,000원은 26.02~08 발행 확정 총액 안에 포함되어 있어 별도 금액으로 더하지 않는다. 수량은 IMD 반영분(53건)만 위 브랜드에 들어가 있다.',
])
r += 4
...
r += 1; s0 = r
YM = [
    ('2025.04', None, None, None, None, None, None, None, None, '2025 계약 진행 (식물나라 51건) · 정산은 26년 발행'),
    ('2025.07', None, None, None, None, None, None, None, None, '2025 계약 진행 (44건) · 정산은 26년 발행'),
    ('2025.08', None, None, 68, None, None, 4739117, None, None, '2025 계약 77건 + 바이오힐보 US 68건 · 정산은 26년 발행'),
    ('2025.09', 1, None, None, None, None, 3034, None, None, 'IMD 집계 시작'),
    ('2025.10', 41, None, None, None, None, 667191, None, None, '정산은 26년 발행'),
    ('2025.11', 5, None, None, None, None, 33815, None, None, '정산은 26년 발행'),
    ('2025.12', 6, None, None, None, None, 21389, None, None, ''),
    ('2026.01', 131, None, None, None, None, 3282085, 8141598, 36584071, '정산은 26.02 이후 발행'),
    ...
    ('(발행월 미확정)', None, None, None, -30000000, 30000000, None, None, None, '바이오힐보 US 30,000,000 국가 재배분 (금액 총계 불변)'),
...
line(ws, r, ['합계'] + [f'=SUM({get_column_letter(c)}{s0}:{get_column_letter(c)}{r-1})'
     ...
     [f'=IFERROR(K{r}/J{r},"")', '금액 849,418,264 = 발행 확정 840,514,264 + JP 미발행 8,904,000'],
     fm, fill=FT, bold=True, wrap=13)
r += 2
note(ws, r, [
    '· KR 금액 = 해당 월 세금계산서 확정 총액 − 글로벌 금액. 글로벌 = JP 시딩 라인 고정비 + 바이오힐보 US.',
    '· 26.02~08 월별 금액은 26.09.07 확정 총액표를 그대로 반영했고, 이 7개월분이 세금계산서 발행액 전부다. 2025 진행분 정산·바이오힐보 US도 이 안에 포함된다.',
    '· 바이오힐보 US 30,000,000원은 발행월이 특정되지 않아 국가 배분만 별도 행으로 조정했다(KR −30,000,000 / 글로벌 +30,000,000, 총계 불변).',
...
for a, b, c_, d in [
    ('세금계산서 총액 (발행 확정)', 840514264, '26.02~08 발행분이 전부. 2025 진행분 정산 59,830,000 · 바이오힐보 US 30,000,000도 이 안에 포함', '— 아래 참조'),
    ('  (기존 집계 대비)', -90773811, '기존 931,288,075은 25년 발행분 89,830,000을 별도로 더한 값 → 확정 총액에 이미 포함되어 있어 중복', ''),
...
r += 1
r = note(ws, r, [
    '· 분자·분모가 같은 자료에서 함께 확정되는 항목만 사용 (전체 청구액의 65%, 진행 1,770건).',
    '· 검증 1 — 아포맨 대조표 296,750,000 + 지급대행 5,000,000 = 세금계산서 발행 2~7월 301,750,000 (일치).',
    '· 검증 2 — 아포맨 원고료 272,248원 × 마크업 1.25 = 340,310원 = 실제 청구 단가 (일치).',
    '· 총액 840,514,264 ÷ 2,139 = 392,948원은 건당 단가가 아니다. 분자에 비시딩 항목이 있고, 청구 대응 진행 건수(약 2,787건)가 분모보다 크다.',
])
r += 1
...
line(ws, r, ['세금계산서 총액 (발행 확정)', '26.02~08', None, None, None, None, None, None, None,
             f'=J{fix_r}'], {10: MON}, fill=FT, bold=True); r += 1
line(ws, r, ['  글로벌 · 일본(JP) 발행분', '일본(JP)', None, None, None, None, None, None, None, 31796000],
     {10: MON}); r += 1
line(ws, r, ['  글로벌 · 바이오힐보 US', '미국(US)', None, None, None, None, None, None, None, 30000000],
     {10: MON}); r += 1
line(ws, r, ['  국내 (KR) = 총액 − 글로벌', '국내', None, None, None, None, None, None, None,
             f'=J{r-3}-J{r-2}-J{r-1}'], {10: MON}, fill=FA, bold=True); r += 1
...
line(ws, r, ['진행 기준 총계', '', None, None, None, None, None, None, None,
             f'=J{r-5}+J{r-1}'], {10: MON}, fill=FT, bold=True)
r += 2
note(ws, r, [
    '· 26.02~08 확정 총액 840,514,264원이 세금계산서 발행액 전부다. 2025 진행분 정산(59,830,000)과 바이오힐보 US(30,000,000)도 이 안에 포함되어 있어 별도로 더하지 않는다.',
    '· 기존 집계 931,288,075원은 위 두 항목을 확정 총액에 다시 더한 값이었다 (중복 89,830,000). 정정 후 −90,773,811원.',
    '· 확정 반영으로 빠진 것 — 26.02 아이디얼포맨 계상분 71,375,000원 (확정 2월 92,750,000원은 루테카 단독과 일치).',
    '· 프로젝트별 내역(②) 대비 차액 70,431,189원 — 2025 계약 정산분·US 청구분(합 89,830,000)의 일부가 여기에, 나머지는 이미 ②의 프로젝트 라인에 계상된 것으로 보인다. 정확한 귀속은 확인 필요.',
...
# ════════════════════════════════ A4_확인필요
ws = sheet('A4_확인필요', '[부속] 확인 필요 항목', '자료 간 불일치 · 미확정 사항 12건 (해결 3건 포함) · 26.09.07 갱신', 'FFC0504D')
widths(ws, [5, 24, 78, 16, 10])
r = 4
header(ws, r, ['#', '항목', '내용', '금액 영향', '우선순위'], 20); r += 1
GAPS = [
    (1, '확정 조정액 브랜드 귀속', '발행 확정 총액이 프로젝트별 내역보다 70,431,189원 많다(US 30,000,000 제외 시 40,431,189). 2025 계약 정산분·US 청구분 포함 여부와 프로젝트별 귀속 확인 필요', 70431189, '높음'),
    (2, '청구 대응 진행 건수', '세금계산서에 수량 미기재 → 840,514,264원에 대응하는 진행 건수 확정 불가(추정 약 2,787건). 확정 시 건당 단가 확정 가능', None, '높음'),
