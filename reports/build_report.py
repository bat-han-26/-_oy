...
r = sec(ws, r, '표준 기준 (⑤)  —  원고료 기준 vs 세금계산서 기준')
header(ws, r, ['비용 기준', '비용', '수량', '조회수', 'CPV', '참여', 'CPE', 'ER', '건당 비용'], 24)
r += 1
for nm, cost, q, memo in [
    ('① 크리에이터 원고료', 492408205, 2086, '마크업·VAT 제외 · 콘텐츠 효율 지표'),
    ('② 세금계산서 발행액', 900344264, 2139, '마크업·VAT·솔루션·지급대행 포함 · 투입 규모'),
]:
    line(ws, r, [nm, cost, q, 56612765, f'=B{r}/D{r}', 390370, f'=B{r}/F{r}',
                 f'=F{r}/D{r}', f'=B{r}/C{r}'],
         {2: MON, 3: CNT, 4: CNT, 5: '#,##0.00"원"', 6: CNT, 7: MON, 8: PCT, 9: MON}, wrap=9); r += 1
line(ws, r, ['원고료 비중', '=B{}/B{}'.format(r-2, r-1), None, None, None, None, None, None,
             '차액 407,936,059원 = 마크업 25% + VAT + 솔루션 + 지급대행 + 성과 미집계 건'],
     {2: PCT}, fill=FT, bold=True, wrap=9)
r += 2
note(ws, r, [
    '[모수] 표준 기준 = 수량·비용 전체 2,139건 / 조회수·참여·CPV·CPE 성과 집계분 2,086건 (05_기준정의 ① 참조)',
    ...
    '[참고] 좋아요 데이터 공란 505건(조회수 8,260,641 · 24.6%) 보정 시 참여 469,050 · ER 0.83% · CPE 1,050원(원고료) / 1,920원(청구). 공식 수치는 미보정값 (A4-3)',
])
ws.freeze_panes = 'A4'

# ════════════════════════════════ 02_브랜드별
ws = sheet('02_브랜드별', '브랜드별 시딩 · 비용 · 성과',
           '표준 기준 ⑤ · 수량 전체 2,139건 / 성과 집계분 2,086건 (바이오힐보 국내·해외 분리)', DEEP)
...
for a, b, c_, d in [
    ...
    ('조회수 · 참여 · CPV · CPE', '성과 집계분 2,086건',
     ...
     'CPV 8.70원 · CPE 1,261원 · ER 0.69% · 건당 조회수 27,139'),
    ('광고 성과', '2,086건 중 집행분', '파트너십 457 + 영상가공 125 = 582 소재', 'ROAS · CPA · 매출'),
    ('(참고) 좋아요 보정', '2,086건',
     '좋아요 공란 505건을 브랜드별 좋아요율·좋아요/댓글비 중앙값으로 추정 → 공식 수치 아님',
     '참여 469,050 · ER 0.83% · CPE 1,050원'),
]:
...
for a, b, c_ in [('CPV (원고료)', '원고료 492,408,205 ÷ 조회수 56,612,765 · 표준 기준 ⑤', '8.70원'),
                 ('CPV (청구)', '세금계산서 900,344,264 ÷ 조회수 56,612,765', '15.90원'),
                 ('CPE (원고료)', '원고료 492,408,205 ÷ 참여 390,370', '1,261원'),
                 ('CPE (청구)', '세금계산서 900,344,264 ÷ 참여 390,370', '2,306원'),
                 ('ROAS', '전환 매출 ÷ 광고비', '파트너십 544% / 합산 530%'),
                 ('CPA', '광고비 ÷ 구매건수', '3,409원'),
                 ('ER (참여율)', '(좋아요 + 댓글) ÷ 조회수 · 표준 기준 ⑤', '0.69%')]:
