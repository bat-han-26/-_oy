...
ws = sheet('01_요약', '올리브영 PB 시딩 성과 요약',
           '집계 26.09.07 · 시딩 2,139건 · 청구 900,344,264원 · CPV 15.9원 · 조회수 56,612,765 · 매출 1,420,018,525원', DEEP)
...
BD = [
    ...
    ('브링그린', 313, 0, 86150000, 15535558, .003821, 124304394, 729266288, '광고비 44% 집행 · 매출 46% 창출'),
    ('라운드어라운드', 281, 0, 67362500, 2413068, .019547, 133848, 348140, '참여율 1위 · 광고 미집행'),
    ...
    ('컬러그램', 71, 32, 20225000, 2653321, .005475, 23514230, 134432958, 'IMD 26.09.07로 103건 확정(v8 77건 → +26)'),
...
for title, hdr, rows_, unit, memo in [
    ...
    for nm, c_, v in rows_:
        base = 2112 if title == '조회수 분포' else 2086
...
r += 1
note(ws, r, [
    '· 파트너십 광고 단독 ROAS 539%, 영상가공 포함 시 527%. 총 구매 86,740건 · CPA 3,269원.',
...
r = sec(ws, r, '③ 참고 — 정합 표본 1,770건 실단가 290,531원 (건당 청구액 420,918원과의 차이)')
...
r = note(ws, r, [
    ...
    '· 표준 기준 ⑤의 건당 청구액 415,863원(총액 ÷ 2,165건)과 위 실단가 290,531원의 차이는 분자에 비시딩 항목이 있고 청구 대응 진행 건수(약 2,787건)가 2,139보다 크기 때문이다. 보고용 지표는 420,918원, 실제 시딩 단가 감각은 290,531원으로 본다.',
...
out = '/home/user/-_oy/reports/올리브영_시딩_성과보고서_260907 HAN.xlsx'
