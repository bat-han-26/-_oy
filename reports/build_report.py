...
KPI = [
    ...
    ('■ 비용', [('세금계산서 총액', 900344264, MON, '원'), ('건당 청구액', 420918, MON, '원'),
                ('(구성) 크리에이터 원고료', 492408205, MON, '원'), ('원고료 비중', 0.5469, PCT, ''),
                ('파트너십 광고비', 267691682, MON, '원')]),
    ('■ 성과', [('총 조회수', 56612765, CNT, '뷰'), ('총 참여 (좋아요+댓글)', 468498, CNT, ''),
                ('총 전환 매출', 1420018525, MON, '원'), ('파트너십 ROAS', 5.4387, RAT, ''),
                ('CPV / CPE', '15.9원 / 1,922원', None, '')]),
]
...
r += 1
line(ws, r, ['세금계산서 발행 확정', 900344264, 2139, 56612765, f'=B{r}/D{r}', 468498,
             f'=B{r}/F{r}', f'=F{r}/D{r}', f'=B{r}/C{r}'],
     {2: MON, 3: CNT, 4: CNT, 5: '#,##0.00"원"', 6: CNT, 7: MON, 8: PCT, 9: MON},
     fill=FT, bold=True, wrap=9); r += 1
line(ws, r, ['  (구성) 크리에이터 원고료', 492408205, None, None, None, None, None, f'=B{r}/B{r-1}',
             '마크업·VAT 제외 · 청구액의 54.7%'], {2: MON, 8: PCT}, wrap=9); r += 1
...
r += 2
note(ws, r, [
    '[모수] 표준 기준 ⑤ = 수량·비용 전체 2,139건 / 조회수·참여 성과 집계분 2,086건 · 모든 단가는 세금계산서 발행 비용 기준 (05_기준정의 ①)',
    ...
    '[참여수] 좋아요 데이터 공란 505건(24.6%)은 브랜드·국가별 좋아요율·좋아요/댓글비 중앙값으로 추정해 반영 — 실측 358,615 + 추정 78,128 = 좋아요 436,743 (A4-3)',
])
ws.freeze_panes = 'A4'

# ════════════════════════════════ 02_브랜드별
ws = sheet('02_브랜드별', '브랜드별 시딩 · 비용 · 성과',
           '표준 기준 ⑤ · 세금계산서 발행 비용 기준 · 수량 2,139건 / 성과 집계분 2,086건 · 참여수는 좋아요 공란 505건 추정 반영', DEEP)
...
BD = [
    ...
    ('바이오힐보 (해외·JP)', 0, 7, 0, 44255, .003344, 0, 0, '26.06 JP 나노 7건 · 웨이크메이크 청구에 포함'),
    ...
    ('컬러그램', 45, 32, 20225000, 1583237, .006809, 7670607, 61405809, 'ROAS 2위'),
...
line(ws, r, ['소계'] + [f'=SUM({get_column_letter(c)}{s0}:{get_column_letter(c)}{r-1})' for c in (2, 3, 4, 5)] +
     [f'=IFERROR(E{r}/D{r},"")', f'=SUM(G{s0}:G{r-1})', f'=IFERROR((56612765*0+0.008276),"")'] +
...
note(ws, r, [
    ...
    '· 참여율은 좋아요 공란 505건(24.6%)을 브랜드·국가별 추정치로 채운 값이다. 실측만으로는 소계 0.69%, 보정 후 0.83% (A4-3).',
...
r += 1; s0 = r
BOH = [
    ('국내 (KR)', 195, 195, 5440064, 26616, 2890, 66989232, '최대 764,503뷰 · 좋아요 실측 22,032 + 추정 4,584'),
    ('해외 · 일본 (JP)', 7, 7, 44255, 143, 5, 2944636, '26.06 나노 시딩 테스트 · 최대 11,572뷰'),
...
ws = sheet('04_광고성과', '파트너십 광고 성과 · 콘텐츠 분포',
           '표준 기준 ⑤ · 조회수 분포 모수 2,086건 · 광고 집행은 2,050건 중 582 소재', DEEP)
...
for a, b, c_, d in [
    ('수량 · 비용 총액', '전체 2,139건', 'IMD 2,050 + JP 26.09 21 + 바이오힐보 US 68', '시딩 건수 · 청구 금액 · 건당 단가'),
    ('조회수 · 참여 · CPV · CPE', '성과 집계분 2,086건',
     'IMD 2,050 + 바이오힐보 US 36 · 미집계 53건(US 32 + JP 21)은 분모 제외 → 표준 기준 ⑤',
     'CPV 15.90원 · CPE 1,922원 · ER 0.83% · 건당 조회수 27,139'),
    ('비용', '세금계산서 발행액 900,344,264원',
     '모든 단가(CPV·CPE·건당)는 세금계산서 발행 비용 기준. 크리에이터 원고료 492,408,205원은 청구액의 54.7% 구성 항목으로만 표기',
     'CPV · CPE · 건당 청구액 420,918원'),
    ('참여수', '실측 358,615 + 추정 78,128',
     '좋아요 공란 505건(24.6%)을 브랜드·국가별 좋아요율과 좋아요/댓글비의 중앙값으로 추정해 반영. 조회수·댓글은 전량 실측',
     '좋아요 436,743 + 댓글 31,755 = 참여 468,498'),
    ('광고 성과', '2,086건 중 집행분', '파트너십 457 + 영상가공 125 = 582 소재', 'ROAS · CPA · 매출'),
]:
...
for a, b, c_, d in [
    ...
    ('  = 진행 기준 총계', 909248264, '', ''),
    ('(구성) 크리에이터 원고료', 492408205, '마크업·VAT 제외 · IMD 2,050건 + 케어플러스 3,700,000 · 청구액의 54.7%', '단가 산정에는 사용하지 않음'),
...
r = note(ws, r, [
    ...
    '· 표준 기준 ⑤의 건당 청구액 420,918원(총액 ÷ 2,139건)과 위 실단가 290,531원의 차이는 분자에 비시딩 항목이 있고 청구 대응 진행 건수(약 2,787건)가 2,139보다 크기 때문이다. 보고용 지표는 420,918원, 실제 시딩 단가 감각은 290,531원으로 본다.',
...
for a, b, c_ in [('CPV', '세금계산서 발행액 900,344,264 ÷ 조회수 56,612,765', '15.90원'),
                 ('CPE', '세금계산서 발행액 900,344,264 ÷ 참여 468,498', '1,922원'),
                 ('건당 청구액', '세금계산서 발행액 900,344,264 ÷ 시딩 2,139건', '420,918원'),
                 ('ER (참여율)', '(좋아요 436,743 + 댓글 31,755) ÷ 조회수 56,612,765', '0.83%'),
...
GAPS = [
    ...
    (3, '좋아요 데이터 공란 505건', 'IMD 2,050건 중 505건(24.6%)의 좋아요 칸이 공란(조회수 8,260,641 · 원고료 127,284,180). 그중 431건은 댓글이 있어 실제 0이 아닌 수집 누락. 보정 시 참여 469,050 · ER 0.83% · CPE 1,050원 → 채우면 성과가 개선되는 방향', None, '높음'),
    (4, 'IMD 국가 분류 오류', 'IMD는 일본 24건(WM 9·CG 8·BOH 7)만 분류하나 실제 99건. 54건(WM 30·CG 24)이 국내로 오분류', None, '높음'),
    (5, '바이오힐보 국내 비용 귀속', '국내 195건·원고료 37,900,000인데 26년 발행액은 6,687,500뿐. 2025 계약 + 브랜드사 직계약으로 분산 추정', None, '높음'),
    (6, '식물나라 소계 오류', '원본 대조표 소계(384건 / 68,177,200 / 95,145,250)가 구버전(370건) 값. 재계산 시 87,676,850 / 106,723,250', 11578000, '높음'),
    (7, '아포맨 누계 행 상충', '요약 행에 194,450,000 / 308,887,500 / 243,062,500 세 값 동시 기재. 월별 상세(237,400,000 / 296,750,000)가 정답', 12137500, '높음'),
    (8, 'JP 마크업률 미확정', 'JP 고정비 29,465,000만 글로벌로 귀속. 세금계산서 42,292,500과의 차액 약 12.8백만(마크업·VAT)은 KR에 잔류', 12827500, '중'),
    (9, '색조 3사 국내분 미발행', 'WM 국내 70건·CG 45건·FM 27건의 국내 시딩 비용이 26년 발행액에 없음. 하반기 계약으로 26.09 이후 발행 예정인지 확인', 34300000, '중'),
    (10, '비시딩 항목 전체 목록', '지급대행·모션그래픽·솔루션 32,400,000원은 확인분만. 전체 목록 확보 시 건당 단가 정밀화', None, '중'),
    (11, '2025년 상반기 데이터', 'IMD가 25.09.29부터라 25년 1~9월 업로드 데이터 없음. 2025 계약 272건 중 219건이 성과 집계 밖', None, '중'),
    (12, '루테카 US 수량', 'US TikTok 약 20건이 IMD·세금계산서 어느 쪽에서도 특정되지 않아 집계 제외', None, '낮음'),
    (13, '바이오힐보 US 미집계 32건', 'US 68건 중 36건만 성과 확보(4,739,117뷰). 나머지 32건 조회수·참여 데이터 미확보 → 실제 성과는 현재 수치 이상', None, '중'),
    (14, '바이오힐보 US 원고료', 'US 36건 원고료 미기재. 청구 30,000,000원만 확인되어 CPV는 청구 기준으로 산출(국내·JP와 기준 상이)', 30000000, '중'),
    (15, '케어플러스 청구 차이', '협의 비용 합 7,825,000(+VAT 8,607,500) vs 세금계산서 8,998,750 → 391,250 차이', 391250, '낮음'),
