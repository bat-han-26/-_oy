...
KPI = [
    ...
    ('■ 비용', [('세금계산서 총액', 900344264, MON, '원'), ('건당 청구액', 415863, MON, '원'),
                ('(구성) 크리에이터 원고료', 494201000, MON, '원'), ('원고료 비중', 0.5489, PCT, ''),
...
for nm, q, a, vw, mth, memo in [
    ...
    ('일본 (JP)', 99, 40511000, 407439, '26.06 ~ 26.09', 'WM 60 · CG 32 건당 415,000 · BOH 7 건당 333,000'),
...
line(ws, r, ['합계', f'=SUM(B{s0}:B{r-1})', f'=B{r}/$B${r}', f'=SUM(D{s0}:D{r-1})',
             ...
             '진행 기준 909,059,264 · 발행 확정 900,344,264 (26년 840,514,264 + 25년 59,830,000) + JP 미발행 8,715,000'],
...
line(ws, r, ['  (구성) 크리에이터 원고료', 494201000, None, None, None, None, None, f'=B{r}/B{r-1}',
             '마크업·VAT 제외 · 청구액의 54.9% · JP는 확정 단가 기준'], {2: MON, 8: PCT}, wrap=9); r += 1
...
BD = [
    ...
    ('웨이크메이크', 70, 60, 22067500, 3143931, .006215, 16409851, 113788050, 'JP 60건 건당 415,000 · 미발행 21건 8,715,000 별도'),
...
r += 1; s0 = r
BOH = [
    ('국내 (KR)', 195, 195, 5440064, 26616, 2890, 66746218, '최대 764,503뷰 · 좋아요 실측 22,032 + 추정 4,584'),
    ('해외 · 일본 (JP)', 7, 7, 44255, 151, 5, 4105157, '26.06 NAD 크림 · 원고료 건당 333,000 · 최대 11,572뷰'),
...
YM = [
    ...
    ('2026.09', 1, 21, None, None, 8715000, 4971, None, None, 'JP 21건 미발행(21×415,000) · 컬러그램 국내 1건'),
]
...
line(ws, r, ['합계'] + [f'=SUM({get_column_letter(c)}{s0}:{get_column_letter(c)}{r-1})'
     ...
     [f'=IFERROR(K{r}/J{r},"")', '금액 909,059,264 = 발행 확정 900,344,264 + JP 미발행 8,715,000'],
     fm, fill=FT, bold=True, wrap=13)
r += 2
note(ws, r, [
    '· KR 금액 = 해당 월 세금계산서 확정 총액 − 글로벌 금액. 글로벌 = JP 확정 단가(WM·CG 415,000 / BOH 333,000) + 바이오힐보 US.',
...
for a, b, c_, d in [
    ...
    ('비용', '세금계산서 발행액 900,344,264원',
     '모든 단가(CPV·CPE·건당)는 세금계산서 발행 비용 기준. 크리에이터 원고료 494,201,000원은 청구액의 54.9% 구성 항목으로만 표기',
...
for a, b, c_, d in [
    ...
    ('  + JP 미발행 (26.09)', 8715000, '웨이크메이크 소프트블러링아이팔레트 21건 × 415,000 · 26.10 발행 예정', ''),
    ('  = 진행 기준 총계', 909059264, '', ''),
    ('(구성) 크리에이터 원고료', 494201000, '마크업·VAT 제외 · 국내는 IMD 기재값, JP는 확정 단가(WM·CG 415,000 / BOH 333,000) · 청구액의 54.9%', '단가 산정에는 사용하지 않음'),
    ('  그중 JP 99건', 40511000, 'WM 60×415,000 + CG 32×415,000 + BOH 7×333,000. IMD 기재값 44,968,205은 라인 총액을 행마다 반복 기재한 값이라 사용하지 않음', '415,000 / 333,000'),
...
GAPS = [
    ...
    (10, 'IMD JP 원고료 기재 오류', 'IMD의 JP 원고료가 라인 총액을 행마다 반복 기재한 값(2,706,390 / 2,255,325이 WM·CG에 동일 등장, 합 44,968,205). 확정 단가 40,511,000으로 교체해 집계했으나 IMD 원본 수정 필요', 4457205, '중'),
