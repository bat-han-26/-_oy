...
ws = sheet('01_요약', '올리브영 PB 시딩 성과 요약',
           '집계 26.09.07 · 시딩 2,165건 · 청구 900,344,264원 · CPV 15.6원 · 조회수 57,682,849 · 매출 1,493,045,674원', DEEP)
widths(ws, [22, 17, 4, 22, 17, 4, 22, 17, 30])
r = 4
KPI = [
    ('■ 규모', [('시딩 건수', 2165, CNT, '건'), ('국내 (KR)', 1998, CNT, '건'),
                ('글로벌 (JP+US)', 167, CNT, '건'), ('브랜드 수', 11, CNT, '개'),
                ('운영 기간', '25.04 ~ 26.09', None, '')]),
    ('■ 비용', [('세금계산서 총액', 900344264, MON, '원'), ('건당 청구액', 415863, MON, '원'),
                ('(구성) 크리에이터 원고료', 498658205, MON, '원'), ('원고료 비중', 0.5539, PCT, ''),
                ('파트너십 광고비', 250300134, MON, '원')]),
    ('■ 성과', [('총 조회수', 57682849, CNT, '뷰'), ('총 참여 (좋아요+댓글)', 472252, CNT, ''),
                ('총 전환 매출', 1493045674, MON, '원'), ('파트너십 ROAS', 5.3939, RAT, ''),
                ('CPV / CPE', '15.6원 / 1,907원', None, '')]),
]
...
for nm, q, a, vw, mth, memo in [
    ('국내 (KR)', 1998, 838548264, 52536293, '25.04 ~ 26.09', '올영PB 11개 브랜드 · 2025 계약 272건 포함'),
    ('일본 (JP)', 99, 40700000, 407439, '26.06 ~ 26.09', '웨이크메이크 60 · 컬러그램 32 · 바이오힐보 7'),
...
line(ws, r, ['합계', f'=SUM(B{s0}:B{r-1})', f'=B{r}/$B${r}', f'=SUM(D{s0}:D{r-1})',
             ...
             '진행 기준 909,248,264 · 발행 확정 900,344,264 (26년 840,514,264 + 25년 59,830,000) + JP 미발행 8,904,000'],
     {2: CNT, 3: PCT, 4: MON, 5: PCT, 6: MON, 7: CNT}, fill=FT, bold=True, wrap=9)
r += 2

r = sec(ws, r, '표준 기준 (⑤)  —  세금계산서 발행 비용 기준')
header(ws, r, ['항목', '금액 / 값', '수량', '조회수', 'CPV', '참여', 'CPE', 'ER', '건당 비용'], 24)
r += 1
line(ws, r, ['세금계산서 발행 확정', 900344264, 2165, 57682849, f'=B{r}/D{r}', 472252,
             f'=B{r}/F{r}', f'=F{r}/D{r}', f'=B{r}/C{r}'],
     {2: MON, 3: CNT, 4: CNT, 5: '#,##0.00"원"', 6: CNT, 7: MON, 8: PCT, 9: MON},
     fill=FT, bold=True, wrap=9); r += 1
line(ws, r, ['  (구성) 크리에이터 원고료', 498658205, None, None, None, None, None, f'=B{r}/B{r-1}',
             '마크업·VAT 제외 · 청구액의 55.4%'], {2: MON, 8: PCT}, wrap=9); r += 1
...
r += 2
note(ws, r, [
    '[모수] 표준 기준 ⑤ = 수량·비용 전체 2,165건 / 조회수·참여 성과 집계분 2,112건 · 모든 단가는 세금계산서 발행 비용 기준 (05_기준정의 ①)',
    ...
    '[참여수] 좋아요 데이터 공란 519건은 브랜드·국가별 좋아요율·좋아요/댓글비 중앙값으로 추정해 반영 — 실측 359,877 + 추정 80,292 = 좋아요 440,169 · 댓글 32,083 (A4-3)',
])
ws.freeze_panes = 'A4'

# ════════════════════════════════ 02_브랜드별
ws = sheet('02_브랜드별', '브랜드별 시딩 · 비용 · 성과',
           '표준 기준 ⑤ · 세금계산서 발행 비용 기준 · 수량 2,165건 / 성과 집계분 2,112건 · 컬러그램 IMD 26.09.07 반영', DEEP)
...
r += 1; s0 = r
BD = [
    ('아이디얼포맨', 737, 0, 262375000, 17330588, .013334, 33768439, 102736712, '2월 계상 71,375,000 확정 제외'),
    ('브링그린', 313, 0, 86150000, 15535558, .003821, 124304394, 729266288, '전체 광고비 44% 집행 · 매출 49% 창출'),
    ('라운드어라운드', 281, 0, 67362500, 2413068, .019547, 133848, 348140, '참여율 1위 1.95% · 광고는 281건 중 1건만 집행'),
    ('바이오힐보 (국내)', 195, 0, 6687500, 5440064, .005424, 56333229, 172219558, '국내 195건'),
    ('바이오힐보 (해외·JP)', 0, 7, 0, 44255, .003525, 0, 0, '26.06 JP 나노 7건 · 웨이크메이크 청구에 포함'),
    ('바이오힐보 (해외·US)', 0, 68, None, 4739117, .005093, 0, 0, '25.08 US 68건 중 36건 성과 확보 · 청구 30,000,000은 2025 계약 정산분에 포함'),
    ('식물나라', 140, 0, 170441825, 2788821, .008976, 14985030, 118255596, ''),
    ('웨이크메이크', 70, 60, 22067500, 3143931, .006215, 16409851, 113788050, 'JP 미발행 8,904,000 별도'),
    ('루테카', 124, 0, 122312500, 1655285, .008485, 0, 0, '광고 미집행 · 브랜드 fade out'),
    ('컬러그램', 71, 32, 20225000, 2653321, .005475, 23514230, 134432958, 'IMD 26.09.07로 103건 확정(v8 77건 → +26) · ROAS 572% 5위'),
    ('케어플러스', 32, 0, 8998750, 188488, .009757, 0, 0, '26.07 신규 · 원고료 3,700,000 확인'),
    ('필리밀리', 27, 0, 3462500, 1353401, .001780, 14086284, 121998372, 'ROAS 1위 · CPA 최저'),
...
line(ws, r, ['소계'] + [f'=SUM({get_column_letter(c)}{s0}:{get_column_letter(c)}{r-1})' for c in (2, 3, 4, 5)] +
     [f'=IFERROR(E{r}/D{r},"")', f'=SUM(G{s0}:G{r-1})', f'=IFERROR((57682849*0+0.008187),"")'] +
...
note(ws, r, [
    ...
    '· 성과(조회수·참여)는 성과 집계분 2,112건에서 발생. 미집계 53건 = US 32 + JP 26.09 21.',
    '· 바이오힐보 US 68건의 청구 30,000,000원은 2025 시딩 계약 행에 포함돼 있다. 조회수는 성과 확보 36건분(4,739,117뷰)이며 건당 조회수는 국내의 4.7배.',
    '· 참여율은 좋아요 공란 519건을 브랜드·국가별 추정치로 채운 값이다. 실측만으로는 소계 0.68%, 보정 후 0.82% (A4-3).',
    '· 컬러그램은 IMD 26.09.07 최신본으로 103건 확정(v8 77건은 부분집합, URL 대조로 확인). 조회수 +68% · 광고비 3.1배 · 매출 2.2배 상향.',
...
header(ws, r, ['구분', '시딩 건수', '성과 확보', '조회수', '건당 조회수', '좋아요', '댓글',
               '참여수', '참여율', '세금계산서', 'CPV', '비고'], 22)
r += 1; s0 = r
BOH = [
    ('국내 (KR)', 195, 195, 5440064, 26616, 2890, 66149614, '최대 764,503뷰 · 좋아요 실측 22,032 + 추정 4,584'),
    ('해외 · 일본 (JP)', 7, 7, 44255, 151, 5, 2907729, '26.06 나노 시딩 테스트 · 최대 11,572뷰'),
    ('해외 · 미국 (US)', 68, 36, 4739117, 23068, 1066, 30000000, '25.08 빅빙세일 · 최대 2,954,129뷰 = 전체 1위'),
]
...
line(ws, r, ['합계'] + [f'=SUM({get_column_letter(c)}{s0}:{get_column_letter(c)}{r-1})' for c in (2, 3, 4)] +
     ...
     [f'=IFERROR(H{r}/D{r},"")', f'=SUM(J{s0}:J{r-1})', f'=IFERROR(J{r}/D{r},"")',
      '해외 75건이 전체 조회수의 46.8%'], fmb, fill=FT, bold=True, wrap=12)
r += 2
note(ws, r, [
    '· 해외 75건(전체의 27.8%)이 바이오힐보 총 조회수 10,223,436뷰의 46.8%를 만들었다. 건당 조회수는 US 131,642뷰 vs 국내 27,898뷰.',
    '· 세금계산서 금액은 발행 총액 900,344,264원을 원고료 비율로 안분한 값이다(US 68건은 실제 청구 30,000,000원 직접 계상). 발행 내역상 브랜드 귀속이 미확정인 조정액이 있어 안분으로 처리했다.',
    '· 국내 좋아요는 실측 22,032 + 추정 4,584(공란 33건), JP는 실측 24 + 추정 119(공란 6건). US는 전량 실측.',
...
YM = [
    ...
    ('2026.08', 223, 50, None, 159644250, 20750000, 7087651, 50022772, 221642299, '발행 확정 180,394,250 · 컬러그램 +25건'),
    ('(국가 재배분)', None, None, None, -30000000, 30000000, None, None, None, '바이오힐보 US 30,000,000 · 2025 계약 금액에 포함되어 있어 KR→글로벌 이동 (총계 불변)'),
    ('2026.09', 1, 21, None, None, 8904000, 4971, None, None, 'JP 21건 세금계산서 미발행 · 컬러그램 국내 1건'),
]
...
ws = sheet('04_광고성과', '파트너십 광고 성과 · 콘텐츠 분포',
           '표준 기준 ⑤ · 조회수 분포 모수 2,112건 · 파트너십 478 + 영상가공 169 = 647 소재', DEEP)
...
for nm, n, c_, v, b, memo in [
    ('파트너십 광고', 478, 250300134, 1350064964, 77032, '컬러그램 IMD 26.09.07 반영'),
    ('영상 가공', 169, 33235171, 142980710, 9708, '컬러그램 IMD 26.09.07 반영'),
]:
...
# ROAS TOP은 광고비 10만원 이상 집행 253건 내 순위
r += 1
...
r += 1
note(ws, r, [
    '· 파트너십 광고 단독 ROAS 539.4%, 영상가공 포함 시 526.6%. 총 구매 86,740건 · CPA 3,269원.',
    '· 파트너십 성과 539.4%는 국내 1,871건(광고 집행 브랜드 8개)에서 발생. 글로벌 77건과 미집행 3개 브랜드(루테카·케어플러스·올더베러) 164건은 광고비·매출이 모두 0이라 제외해도 ROAS는 동일하다.',
    '· ROAS TOP 5는 광고비 10만원 이상 집행 253건 내 순위다. 소액 집행 건은 배수가 과대해져 제외했다.',
...
for a, b, c_, d in [
    ('수량 · 비용 총액', '전체 2,165건', 'IMD 2,076(컬러그램 26.09.07 반영) + JP 26.09 21 + 바이오힐보 US 68', '시딩 건수 · 청구 금액 · 건당 청구액'),
    ('조회수 · 참여 · CPV · CPE', '성과 집계분 2,112건',
     'IMD 2,076 + 바이오힐보 US 36 · 미집계 53건(US 32 + JP 21)은 분모 제외 → 표준 기준 ⑤',
     'CPV 15.61원 · CPE 1,907원 · ER 0.82% · 건당 조회수 27,312'),
    ('비용', '세금계산서 발행액 900,344,264원',
     '모든 단가(CPV·CPE·건당)는 세금계산서 발행 비용 기준. 크리에이터 원고료 498,658,205원은 청구액의 55.4% 구성 항목으로만 표기',
     'CPV · CPE · 건당 청구액 415,863원'),
    ('참여수', '실측 359,877 + 추정 80,292',
     '좋아요 공란 519건을 브랜드·국가별 좋아요율과 좋아요/댓글비의 중앙값으로 추정해 반영. 조회수·댓글은 전량 실측',
     '좋아요 440,169 + 댓글 32,083 = 참여 472,252'),
    ('광고 성과', '2,112건 중 집행분', '파트너십 478 + 영상가공 169 = 647 소재', 'ROAS · CPA · 매출'),
]:
...
for a, b, c_, d in [
    ('세금계산서 총액 (발행 확정)', 900344264, '26.02~08 확정 840,514,264 + 25년 발행 59,830,000 (바이오힐보 US 30,000,000 포함)', '415,863원 ≠ 건당단가'),
    ...
    ('  = 진행 기준 총계', 909248264, '', ''),
    ('(구성) 크리에이터 원고료', 498658205, '마크업·VAT 제외 · IMD 2,076건 + 케어플러스 3,700,000 · 청구액의 55.4%', '단가 산정에는 사용하지 않음'),
    ('비시딩 항목 (확인분)', 32400000, '지급대행 25,200,000 · 모션그래픽 5,000,000 · 솔루션 2,000,000 · 풀필먼트 200,000', ''),
]:
    line(ws, r, [a, b, c_, d], {2: MON}, wrap=3, h=17); r += 1
r += 1

r = sec(ws, r, '③ 참고 — 정합 표본 1,770건 실단가 290,531원 (건당 청구액 415,863원과의 차이)')
...
r = note(ws, r, [
    ...
    '· 표준 기준 ⑤의 건당 청구액 415,863원(총액 ÷ 2,165건)과 위 실단가 290,531원의 차이는 분자에 비시딩 항목이 있고 청구 대응 진행 건수(약 2,813건)가 2,165보다 크기 때문이다. 보고용 지표는 415,863원, 실제 시딩 단가 감각은 290,531원으로 본다.',
...
for a, b, c_ in [('CPV', '세금계산서 발행액 900,344,264 ÷ 조회수 57,682,849', '15.61원'),
                 ('CPE', '세금계산서 발행액 900,344,264 ÷ 참여 472,252', '1,907원'),
                 ('건당 청구액', '세금계산서 발행액 900,344,264 ÷ 시딩 2,165건', '415,863원'),
                 ('ER (참여율)', '(좋아요 440,169 + 댓글 32,083) ÷ 조회수 57,682,849', '0.82%'),
                 ('ROAS', '전환 매출 ÷ 광고비', '파트너십 539% / 합산 527%'),
                 ('CPA', '광고비 ÷ 구매건수', '3,269원')]:
    line(ws, r, [a, b, c_, ''], wrap=2); r += 1
r += 1
note(ws, r, [
    '출처 — 세금계산서 월별 확정 총액표(26.09.07) · 2025 시딩 정산표 · 브랜드별 월별 대조표 · 글로벌 시딩 진행 라인 · IMD 성과 통계 v8(26.09.04) · IMD 컬러그램(26.09.07) 103행 · IMD 바이오힐보 US(26.09.07) 36행 · 케어플러스 시딩현황',
...
GAPS = [
    ...
    (2, '청구 대응 진행 건수', '세금계산서에 수량 미기재 → 900,344,264원에 대응하는 진행 건수 확정 불가(추정 약 2,813건). 확정 시 건당 단가 확정 가능', None, '높음'),
    (3, '좋아요 데이터 공란 519건', 'IMD 2,076건 중 519건(25.0%)의 좋아요 칸이 공란. 대부분 댓글이 있어 실제 0이 아닌 수집 누락. 브랜드·국가별 추정치로 반영(80,292) — 채워지면 실측으로 대체', None, '높음'),
    ...
    (9, 'JP 마크업률 미확정', 'JP 고정비 29,465,000만 글로벌로 귀속. 세금계산서 42,292,500과의 차액 약 12.8백만(마크업·VAT)은 KR에 잔류', 12827500, '중'),
    (10, '색조 3사 국내분 미발행', 'WM 국내 70건·CG 45건·FM 27건의 국내 시딩 비용이 26년 발행액에 없음. 하반기 계약으로 26.09 이후 발행 예정인지 확인', 34300000, '중'),
    (11, '비시딩 항목 전체 목록', '지급대행·모션그래픽·솔루션 32,400,000원은 확인분만. 전체 목록 확보 시 건당 단가 정밀화', None, '중'),
    (12, '2025년 상반기 데이터', 'IMD가 25.09.29부터라 25년 1~9월 업로드 데이터 없음. 2025 계약 272건 중 219건이 성과 집계 밖', None, '중'),
    (13, '루테카 US 수량', 'US TikTok 약 20건이 IMD·세금계산서 어느 쪽에서도 특정되지 않아 집계 제외', None, '낮음'),
    (14, '바이오힐보 US 미집계 32건', 'US 68건 중 36건만 성과 확보(4,739,117뷰). 나머지 32건 조회수·참여 데이터 미확보 → 실제 성과는 현재 수치 이상', None, '중'),
    (15, '바이오힐보 US 원고료', 'US 36건 원고료 미기재. 청구 30,000,000원만 확인되어 CPV는 청구 기준으로 산출(국내·JP와 기준 상이)', 30000000, '중'),
    (16, '케어플러스 청구 차이', '협의 비용 합 7,825,000(+VAT 8,607,500) vs 세금계산서 8,998,750 → 391,250 차이', 391250, '낮음'),
...
out = '/home/user/-_oy/reports/올리브영_시딩_성과보고서_260907 HAN (최종).xlsx'
