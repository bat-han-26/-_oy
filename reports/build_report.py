...
ws = sheet('01_요약', '올리브영 PB 시딩 성과 요약',
           '집계 26.09.07 · 시딩 2,139건 · 청구 930,344,264원(확정) · 조회수 56,612,765 · 매출 1,420,018,525원', DEEP)
...
KPI = [
    ...
    ('■ 비용', [('세금계산서 총액', 930344264, MON, '원'), ('크리에이터 원고료', 492408205, MON, '원'),
...
for nm, q, a, vw, mth, memo in [
    ('국내 (KR)', 1972, 868548264, 51550844, '25.04 ~ 26.08', '올영PB 11개 브랜드 · 2025 계약 272건 포함'),
...
line(ws, r, ['합계', f'=SUM(B{s0}:B{r-1})', f'=B{r}/$B${r}', f'=SUM(D{s0}:D{r-1})',
             ...
             '진행 기준 총계 · 세금계산서 발행분 930,344,264(확정) + JP 미발행 8,904,000'],
...
note(ws, r, [
    ...
    '[금액] 세금계산서 발행 총액 930,344,264원(26.09.07 확정) 기준 · 건당 청구단가 307,481원은 진행 건수가 확정된 1,770건 가중평균',
...
line(ws, r, ['확정 조정', None, None, None, 70431189, None, None, None, None, None, None,
             '3~8월 발행 확정액 − 기존 내역 차이 · 브랜드 귀속 확인필요 (A1·A4)'], fm, wrap=12)
adj_r = r; r += 1
...
line(ws, r, ['합계', f'=B{sub_r}', f'=C{sub_r}', f'=D{sub_r}', f'=E{sub_r}+E{adj_r}+E{r-1}',
             ...
             f'=IFERROR(J{r}/I{r},"")', '세금계산서 확정 총액 930,344,264원과 일치'],
...
YM = [
    ...
    ('2025.08', None, None, 68, 19315000, 30000000, 4739117, None, None, '2025 계약 77건 + 바이오힐보 US 68건'),
    ...
    ('2026.02', 267, None, None, 92750000, None, 6780868, 16230248, 50750402, '발행 확정 92,750,000'),
...
line(ws, r, ['합계'] + [f'=SUM({get_column_letter(c)}{s0}:{get_column_letter(c)}{r-1})'
     ...
     [f'=IFERROR(K{r}/J{r},"")', '금액 939,248,264 = 발행 확정 930,344,264 + JP 미발행 8,904,000'],
     fm, fill=FT, bold=True, wrap=13)
r += 2
note(ws, r, [
    '· KR 금액 = 해당 월 세금계산서 확정 총액 − 글로벌 금액. 글로벌 = JP 시딩 라인 고정비 + 바이오힐보 US.',
    '· 26.02~08 월별 금액은 26.09.07 확정 총액표를 그대로 반영했다. 기존 내역 대비 차이는 A1 하단 참조.',
...
for a, b, c_, d in [
    ('세금계산서 총액 (발행 확정)', 930344264, '26.02~08 확정 840,514,264 + 25년 발행 89,830,000 (US 30,000,000 + 2025계약 59,830,000)', '— 아래 참조'),
    ('  (기존 집계 대비)', -943811, '26.02 계상 71,375,000 제외 + 26.03~08 재확정 +70,431,189', ''),
...
line(ws, r, ['세금계산서 총액 (발행 확정)', '', None, None, None, None, None, None, None,
             f'=J{r-3}+J{r-2}+J{r-1}'], {10: MON}, fill=FT, bold=True)
r += 2
note(ws, r, [
    '· 26.02~08 금액은 26.09.07 확정 총액표 그대로다. 프로젝트별 내역(②)은 이전 집계 기준이며, 차이는 아직 프로젝트에 귀속되지 않았다.',
    '· 확정 반영으로 빠진 것 — 26.02 아이디얼포맨 계상분 71,375,000원 (확정 2월 92,750,000원은 루테카 단독과 일치).',
    '· 확정 반영으로 더해진 것 — 26.03 +42,075,000 / 26.04 +5,058,689 / 26.05 +22,553,250 / 26.08 +31,740,500, 26.07 −30,996,250 (7→8월 발행 이동으로 추정).',
    '· 순증감 −943,811원 (기존 집계 931,288,075 → 확정 930,344,264).',
...
GAPS = [
    (1, '확정 조정액 브랜드 귀속', '26.03~08 발행 확정액이 기존 내역보다 70,431,189원 많다(03 +42,075,000 / 04 +5,058,689 / 05 +22,553,250 / 07 −30,996,250 / 08 +31,740,500). 프로젝트별 귀속 필요', 70431189, '높음'),
    (2, '청구 대응 진행 건수', '세금계산서에 수량 미기재 → 930,344,264원에 대응하는 진행 건수 확정 불가(추정 약 2,787건). 확정 시 건당 단가 확정 가능', None, '높음'),
