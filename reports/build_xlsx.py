...
ws.cell(row=r, column=1, value='콘텐츠 성과 (전체 2,139건 기준 · 성과 데이터 보유 2,050건)').font = F_LBL
r += 1
...
KPI_L = [
    ...
    ('건당 조회수 (전체 2,139)', 24251, CNT, '성과보유 2,050 기준 25,304'),
    ('시딩 총원고료', 488708205, MONEY_W, '원고료만 · 광고비와 별도'),
    ('CPV', 9.42113, '#,##0.00\\원', '원고료 ÷ 조회수'),
]
KPI_R = [
    ('광고 소재 활용', 582, CNT, '파트너십광고 457 + 영상가공 125'),
    ('시딩 1건당 소재 활용', 0.272090, '0.00', '582 ÷ 2,139'),
...
head(ws, '브랜드별 콘텐츠 성과 (전체 2,139건 기준)',
     ...
     ['브랜드', '시딩 전체', '성과 데이터 보유', '성과 미보유', '총 조회수', '총 좋아요', '총 댓글',
      ...
      'ROAS', 'CPA', '건당 조회수 (전체)', '건당 조회수 (성과보유)', '1건당 소재활용'],
     [15, 10, 12, 10, 12, 10, 9, 9, 15, 10, 10, 14, 9, 15, 8, 10, 12, 13, 11], heights=34)
PERF = [
...
fmp = {2: CNT, 3: CNT, 4: CNT, 5: CNT, 6: CNT, 7: CNT, 8: '0.00%', 9: MONEY_W,
       ...
       15: RATIO, 16: MONEY_W, 17: CNT, 18: CNT, 19: '0.00'}
for nm, qa, qp, vw, lk, cm, fee, mat, ad, buy, rev in PERF:
    row(ws, r, [nm, qa, qp, f'=B{r}-C{r}', vw, lk, cm,
                ...
                f'=IFERROR(E{r}/B{r},"")', f'=IFERROR(E{r}/C{r},"")',
                f'=IFERROR(K{r}/B{r},"")'],
...
row(ws, r, ['전체 합계'] +
    ...
    [f'=IFERROR(N{r}/L{r},"")', f'=IFERROR(L{r}/M{r},"")',
     f'=IFERROR(E{r}/B{r},"")', f'=IFERROR(E{r}/C{r},"")', f'=IFERROR(K{r}/B{r},"")'],
...
r += 1
notes(ws, r, [
    '· 조회수·좋아요·광고비·매출 등 성과 값은 성과 데이터 보유 2,050건에서만 발생한다. 건당 조회수를 전체(2,139) 기준과 성과보유(2,050) 기준 두 열로 나눠 표기했다.',
...
for nm, a, b, memo, f in [
    ...
    ('건당 조회수 — 전체 기준', 27569, 24251, '총 조회수 ÷ 전체 시딩 건수', CNT),
    ('건당 조회수 — 성과보유 기준', 29061, 25304, '총 조회수 ÷ 성과 데이터 보유 건수', CNT),
]:
...
for title, hdr, rowsd, note, money in [
    ('③ 히어로 콘텐츠 분포 (모수 2,139건)', ['구간', '건수', '전체 대비 비중', '해당 구간 조회수 합'],
     [('조회수 50만 이상', 6, 4223705), ('조회수 30만 이상', 17, 8372082),
      ('조회수 10만 이상', 167, 30296912), ('조회수 5만 이상', 255, 36528445)],
     '· 상위 255건(11.9%)이 전체 조회수의 70.4%를 만든다.', False),
    ('④ ROAS 기준 분포 (모수 2,139건 · 광고비 10만원 이상 집행 건만 카운트)',
    ...
    ('⑤ 매출액 기준 분포 (모수 2,139건)', ['구간', '건수', '전체 대비 비중', '해당 구간 매출 합 (원)'],
     ...
     '· 상위 2건(0.09%)이 전체 매출의 9.0%, 상위 42건(2.0%)이 58.6%를 만든다.', True),
]:
    ...
    for nm, cnt, tot in rowsd:
        row(ws, r, [nm, cnt, f'=IFERROR(B{r}/2139,"")', tot],
...
head(ws, '최상위 0.1% 콘텐츠 — 전체 시딩 2,139건 × 0.1% = 상위 2건',
