...
r = 5
summary = [
    ('2026년 발행분 (2~8월)', '국내(KR)',        809662075, '세금계산서 841,458,075 − 글로벌 JP 고정비 31,796,000'),
    ('2025년 시딩 계약 (25.4~26.1 진행)', '국내(KR)', 59830000, '2025 시딩 정산표 (제리와콩나무 72건 + BAT 200건)'),
    ('글로벌 JP 시딩 (26.06~08 발행분)', '일본(JP)', 31796000, '글로벌 JP 라인 고정비 — 26.06 2,331,000 + 26.07 8,715,000 + 26.08 20,750,000'),
    ('2025년 8월 올영PB 바이오힐보 US', '미국(US)', 30000000, '68건 · 세금계산서 총액표 별도 항목'),
]
first = r
for name, ctry, amt, src in summary:
    row(ws, r, [name, ctry, amt, f'=C{r}/$C${first + len(summary)}', src],
        fmts={3: MONEY_W, 4: PCT}, wrap_from=5); r += 1
row(ws, r, ['전체 총계 (세금계산서 발행분)', '', f'=SUM(C{first}:C{r - 1})', f'=C{r}/$C${r}',
            '세금계산서 총액표 전체 총계 931,288,075와 일치'], fill=FILL_T, fmts={3: MONEY_W, 4: PCT})
total_row = r
r += 1
row(ws, r, ['(별도) 글로벌 JP 미발행 — 26.09 진행분', '일본(JP)', 8904000, '',
            '웨이크메이크 소프트블러링아이팔레트 21건 (9/14~) · 세금계산서 미발행'],
    fill=FILL_A, fmts={3: MONEY_W}, wrap_from=5)
r += 1
row(ws, r, ['진행 기준 총계', '', f'=C{total_row}+C{r - 1}', '',
            'KR 869,492,075 + 글로벌 70,700,000 (US 30,000,000 + JP 고정비 40,700,000)'],
    fill=FILL_T, fmts={3: MONEY_W}, wrap_from=5)
r += 2
...
for nm, formula, exp, desc in [
    ...
    ('IMD 2,050건 복원',      f'=B{sum_row}+C17+C18+C19', 2050, 'KR 1,972 + 글로벌 기간 내 78 (26.06 7 + 26.07 21 + 26.08 50)'),
