...
ws.cell(row=r, column=1, value='수량 — 글로벌(JP) 분리 기준').font = F_LBL
r += 1
...
for name, ctry, q, note in [
    ('IMD 업로드 완료 — 재분류 후 국내', '국내', 1972, 'IMD 전체 2,050 − 글로벌(JP) 78'),
    ('IMD 업로드 완료 — 글로벌', '일본(JP)', 78, '글로벌시딩_JP 시트 중 IMD 집계기간(≤26.08.30) 내 건'),
]:
    row(ws, r, [name, ctry, q, '', note], fmts={3: CNTW}, wrap_from=5); r += 1
row(ws, r, ['IMD 합계', '', f'=SUM(C{qty_first}:C{r - 1})', '', '2,050건 (원본 분류: 국내 2,026 / 일본 24 → 글로벌 실적 반영해 재분류)'],
    fill=FILL_T, fmts={3: CNTW}, wrap_from=5)
r += 1
row(ws, r, ['글로벌(JP) — IMD 집계기간 외', '일본(JP)', 21, '', '웨이크메이크 소프트블러링아이팔레트 9/14~ · IMD 2,050건에 미포함'], fmts={3: CNTW}, wrap_from=5)
r += 1
row(ws, r, ['글로벌(JP) 총 진행 수량', '일본(JP)', 99, '', '78건(기간 내) + 21건(기간 외) · 고정비 40,700,000원'], fill=FILL_A, fmts={3: CNTW}, wrap_from=5)
r += 1
...
notes(ws, r, [
    ...
    '· 글로벌(JP) 99건 / 고정비 40,700,000원은 별도 제공 자료 기준이며, IMD의 국가 분류(일본 24건)보다 54건 많다 → IMD가 JP 건 다수를 국내로 잡고 있다 (불일치_확인필요 #11).',
    '· 출처: 올영PB UGC 세금계산서 기준 총액표(26.09.01), 2025 시딩 정산표, 브랜드별 월별 대조표, 글로벌 시딩 라인 자료, IMD 원본데이터 2,050행',
])

# ─────────────────────────────────────────── 1-2. 연월별_수량금액
ws = wb.create_sheet('연월별_수량금액')
head(ws, '연월별 시딩 수량 · 금액 — KR / 글로벌',
     '글로벌 = 근거 자료로 국가가 특정되는 건만 (글로벌 JP 라인 · 바이오힐보 US) · 추정이거나 근거 없는 건은 전부 KR로 계상 · 수량 = 업로드/진행월, 금액 = 세금계산서 발행월',
     ['연월', 'KR 수량', '글로벌 수량', '합계 수량', 'KR 금액', '글로벌 금액', '합계 금액',
      '(참고) 글로벌 JP 고정비', '비고'],
     [10, 10, 11, 11, 15, 14, 15, 17, 48], heights=32)

# (연월, KR수량, 글로벌수량, KR금액, 글로벌금액, 글로벌JP고정비, 비고)
YM = [
    ('2025.04', None, None, 7300000,   None,     None,     '2025 시딩 진행월 · 식물나라 51건은 IMD 집계 밖'),
    ('2025.07', None, None, 12215000,  None,     None,     '2025 시딩 진행월 · WM산리오+ID올인원+BOH판테셀 44건 IMD 밖'),
    ('2025.08', None, 68,   19315000,  30000000, None,     '2025 시딩 WM코팅밤 77건 IMD 밖 / 글로벌 = 바이오힐보 US 68건'),
    ('2025.09', 1,    0,    None,      None,     None,     'IMD 집계 시작 (09-29)'),
    ('2025.10', 41,   0,    11760000,  None,     None,     '2025 시딩 BOH슈링크+WM실버크러시 진행월'),
    ('2025.11', 5,    0,    8190000,   None,     None,     '2025 시딩 식물나라 진행월'),
    ('2025.12', 6,    0,    None,      None,     None,     ''),
    ('2026.01', 131,  0,    1050000,   None,     None,     '2025 시딩 컬러그램 진행월'),
    ('2026.02', 267,  0,    164125000, None,     None,     '아포맨 71,375,000 + 루테카 92,750,000'),
    ('2026.03', 326,  0,    46737500,  None,     None,     '식물나라 12,500,000 + 라운드어라운드 4,675,000 + 루테카 29,562,500'),
    ('2026.04', 313,  0,    124330000, None,     None,     '아포맨 108,000,000 + 식물나라 13,080,000 + 브링그린 3,250,000'),
    ('2026.05', 290,  0,    98428075,  None,     None,     '아포맨 74,125,000 + 식물나라 24,303,075'),
    ('2026.06', 214,  7,    133875000, None,     2331000,  '글로벌 = 바이오힐보JP NAD 크림 7건'),
    ('2026.07', 180,  21,   125308750, None,     8715000,  '글로벌 = WM 10 + CG 11 (7/20~) · KR 금액에 WM·CG 발행액 24,937,500 포함(국가 근거 없어 KR)'),
    ('2026.08', 198,  50,   148653750, None,     20750000, '글로벌 = WM 29 + CG 21 (8/13~) · KR 금액에 WM·CG 발행액 17,355,000 포함(국가 근거 없어 KR)'),
    ('2026.09', 0,    21,   None,      None,     8904000,  '글로벌 = WM 소프트블러링아이팔레트 21건 · IMD 집계 밖'),
]
r = 5; s0 = r
fmy = {2: CNT, 3: CNT, 4: CNT, 5: MONEY_W, 6: MONEY_W, 7: MONEY_W, 8: MONEY_W}
for ym, kq, gq, ka, ga, gf, memo in YM:
    row(ws, r, [ym, kq, gq,
                (f'=SUM(B{r}:C{r})' if (kq is not None or gq is not None) else None),
                ka, ga,
                (f'=SUM(E{r}:F{r})' if (ka is not None or ga is not None) else None),
                gf, memo], fmts=fmy, wrap_from=9)
    ws.row_dimensions[r].height = 26
    r += 1
row(ws, r, ['합계'] + [f'=SUM({get_column_letter(c)}{s0}:{get_column_letter(c)}{r - 1})'
                       for c in (2, 3, 4, 5, 6, 7, 8)] +
    ['수량 2,139건 (KR 1,972 + 글로벌 167) · 금액 931,288,075원 (세금계산서 총액과 일치)'],
...
r += 1
for i, h in enumerate(['항목', '값', '기대값', '판정', '설명'], 1):
    c = ws.cell(row=r, column=i, value=h); c.font, c.fill, c.border = F_HDR, FILL_H, BOX
    c.alignment = Alignment(horizontal='center', vertical='center')
r += 1
row(ws, r, ['IMD 2,050건 복원', f'=B{tot_ym}+E9+E10+E11', 2050, '', ''], fmts={2: CNT, 3: CNT})
ws[f'B{r}'] = f'=B{tot_ym}+C17+C18+C19'
ws[f'D{r}'] = f'=IF(B{r}=C{r},"일치","불일치")'
ws[f'E{r}'] = 'KR 1,972 + 글로벌 기간 내 78 (26.06 7 + 26.07 21 + 26.08 50)'
ws[f'E{r}'].font = F_TXT; ws[f'E{r}'].alignment = Alignment(horizontal='left', vertical='center', wrap_text=True)
r += 1
row(ws, r, ['세금계산서 총액', f'=G{tot_ym}', 931288075, '', '세금계산서 총액표 전체 총계와 일치해야 함'], fmts={2: MONEY_W, 3: MONEY_W}, wrap_from=5)
ws[f'D{r}'] = f'=IF(B{r}=C{r},"일치","불일치")'
r += 1
row(ws, r, ['글로벌 JP 고정비', f'=H{tot_ym}', 40700000, '', '글로벌시딩_JP 총계와 일치해야 함 (합계 금액에는 미포함)'], fmts={2: MONEY_W, 3: MONEY_W}, wrap_from=5)
ws[f'D{r}'] = f'=IF(B{r}=C{r},"일치","불일치")'
r += 2

ws.cell(row=r, column=1, value='2025 시딩 계약 수량 (진행월 · IMD 축과 중복 가능 → 위 합계 미포함)').font = F_LBL
r += 1
for i, h in enumerate(['진행월', '수량', '금액', '', '', '', '', '', '내역'], 1):
    c = ws.cell(row=r, column=i, value=h); c.font, c.fill, c.border = F_HDR, FILL_H, BOX
    c.alignment = Alignment(horizontal='center', vertical='center')
r += 1
c25 = r
for ym, q, a, memo in [
    ...
    row(ws, r, [ym, q, a, None, None, None, None, None, memo], fmts={2: CNT, 3: MONEY_W}, wrap_from=9); r += 1
row(ws, r, ['합계', f'=SUM(B{c25}:B{r - 1})', f'=SUM(C{c25}:C{r - 1})',
            None, None, None, None, None, '272건 / 59,830,000원 · 25.04~25.08 진행분 172건은 IMD 집계 밖'],
    fill=FILL_T, fmts={2: CNT, 3: MONEY_W}, wrap_from=9)
r += 2
notes(ws, r, [
    '[국가 배분 규칙] 글로벌 열에는 근거 자료로 국가가 특정되는 건만 넣었다 — ① 글로벌 시딩 진행 라인(JP) 99건 ② 바이오힐보 US 68건. 그 외 추정·근거 없는 건은 전부 KR로 계상했다.',
    '· 따라서 세금계산서 상 웨이크메이크(22,067,500)·컬러그램(20,225,000)·바이오힐보(6,687,500)·루테카(122,312,500)는 KR 금액에 들어가 있다. 총액표에 국가 구분이 없어 국가 배분 근거가 없기 때문이다.',
    '· 글로벌 JP 고정비(H열)는 마크업·VAT 제외 값이며 세금계산서와 개념이 달라 합계 금액에서 제외했다. 참고용이다.',
    '· 축이 다르다: 수량은 업로드/진행월, 금액은 세금계산서 발행월(2025 시딩분은 진행월). 같은 행의 수량과 금액이 같은 캠페인을 가리키지 않을 수 있다.',
    '· 2026.07~08 KR 수량은 IMD 국내에서 글로벌 오분류분(7월 4건 · 8월 50건)을 차감한 값이다 (IMD 국내 원값 184 / 248).',
    '· 출처: 세금계산서 총액표, 2025 시딩 정산표, 글로벌 시딩 진행 라인 자료, IMD 원본데이터 2,050행.',
])
...
r += 2
ws.cell(row=r, column=1, value='글로벌 자료 반영 재분류').font = F_LBL
r += 1
for i, h in enumerate(['구분', '건수', '비고'], 1):
    c = ws.cell(row=r, column=i, value=h); c.font, c.fill, c.border = F_HDR, FILL_H, BOX
    c.alignment = Alignment(horizontal='center', vertical='center')
r += 1
rc0 = r
row(ws, r, ['국내 (재분류 후)', 1972, 'IMD 전체 2,050 − 글로벌 78'], fmts={2: CNT}); r += 1
row(ws, r, ['글로벌 JP (기간 내)', 78, 'WM 39 · CG 32 · BOH 7'], fmts={2: CNT}); r += 1
row(ws, r, ['IMD 합계', f'=SUM(B{rc0}:B{r - 1})', '2,050건'], fill=FILL_T, fmts={2: CNT}); r += 1
row(ws, r, ['글로벌 JP (기간 외)', 21, '9/14~ 업로드 · IMD 2,050건 미포함'], fmts={2: CNT}); r += 2
notes(ws, r, [
    '· 위 월별 표의 국내/일본은 IMD 원본 국가 컬럼 그대로이며, 일본 24건만 잡혀 있다 (실제 78건).',
...
head(ws, '브랜드별 수량 — 전체에서 글로벌(JP)을 차감한 국내 순수량',
     'IMD 업로드 기준 2,050건 − 글로벌(JP) 78건(집계기간 내) = 국내 1,972건 · 원고료는 마크업·VAT 제외',
     ['브랜드', 'IMD 전체 건수', '글로벌(JP) 차감', '국내 순수량', '글로벌 비중',
      'IMD 시딩 총원고료', '업로드 월', '비고'],
     [16, 13, 13, 12, 10, 17, 24, 40])
bd = [
    ('아이디얼포맨',     737, 0,  186700000, '25.09~26.07', ''),
    ('브링그린',         313, 0,  66300000,  '26.02~26.08', ''),
    ('라운드어라운드',   281, 0,  61550000,  '26.02·03·05·06', ''),
    ('바이오힐보',       202, 7,  39565965,  '25.10~26.08', 'JP = NAD 크림 7건 (IMD 일본 분류와 일치)'),
    ('식물나라',         140, 0,  32490000,  '25.11~26.05', ''),
    ('루테카',           124, 0,  22500000,  '26.01~26.03', 'US TikTok 약 20건은 IMD 미포함'),
    ('웨이크메이크',     109, 39, 38854315,  '25.10 · 26.05~08', 'JP 39건 (7월 10 + 8월 29) · IMD는 9건만 일본으로 분류'),
    ('컬러그램',         77,  32, 32447925,  '26.01 · 26.05~08', 'JP 32건 (7월 11 + 8월 21) · IMD는 8건만 일본으로 분류'),
    ('케어플러스',       32,  0,  0,         '26.07~08', '원고료 미입력'),
    ('필리밀리',         27,  0,  6300000,   '26.05~08', ''),
    ('올더베러',         8,   0,  2000000,   '26.05~06', ''),
    ('딜라이트프로젝트', 0,   0,  0,         '시딩 데이터 없음', ''),
]
r = 5; s0 = r
fmb = {2: CNT, 3: CNT, 4: CNT, 5: PCT, 6: MONEY_W}
for nm, tot, jp, fee, mth, memo in bd:
    row(ws, r, [nm, tot, jp, f'=B{r}-C{r}', f'=IFERROR(C{r}/B{r},"")', fee, mth, memo],
        fill=(FILL_A if jp else None), fmts=fmb, wrap_from=7); r += 1
row(ws, r, ['합계'] + [f'=SUM({get_column_letter(c)}{s0}:{get_column_letter(c)}{r - 1})' for c in (2, 3)] +
    [f'=B{r}-C{r}', f'=IFERROR(C{r}/B{r},"")',
     f'=SUM(F{s0}:F{r - 1})', '', ''], fill=FILL_T, fmts=fmb)
r += 2
notes(ws, r, [
    '· 글로벌(JP) 차감분은 글로벌시딩_JP 시트의 IMD 집계기간 내 78건이다. 기간 외 21건(9/14~ 웨이크메이크)은 IMD 2,050건에 없어 차감하지 않았다.',
    '· IMD 원본의 국가 컬럼은 일본을 24건(WM 9 · CG 8 · BOH 7)만 잡고 있다. 실제 글로벌 자료 기준 78건이므로 54건(WM 30 · CG 24)이 국내로 오분류돼 있다.',
    '· IMD 시딩 총원고료는 국내·JP가 섞인 값이라 국가별로 쪼갤 수 없다. 글로벌 고정비는 글로벌시딩_JP 시트를 볼 것.',
